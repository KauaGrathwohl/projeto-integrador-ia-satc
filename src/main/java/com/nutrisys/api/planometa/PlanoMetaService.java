...
public class PlanoMetaService {
    ...
    public CreatedPlanoMetaDto createPlanoMeta(CreatePlanoMetaDto createPlanoMetaDto, Long idPaciente) {
        ...
        // Finaliza os outros planos do paciente
        List<PlanoMeta> planosPaciente = planoMetaRepository.findByPacienteId(idPaciente);
        for (PlanoMeta plano : planosPaciente) {
            // Não finaliza o plano que foi criado
            if (plano.getId().equals(planoMetaCreated.getId())) {
                continue;
            }

            // Não altera data dos já finalizados
            if (plano.getDtFinalMeta() != null) {
                continue;
            }

            plano.setDtFinalMeta(LocalDate.now());

            planoMetaRepository.save(plano);
        }
    ...
    public FindPlanoMetaDto findPlanoMeta(Long id) {
        PlanoMeta planoMeta = planoMetaRepository.findOneById(id);

        return new FindPlanoMetaDto(
            planoMeta.getDtInicioMeta(),
            planoMeta.getDtFinalMeta(),
            planoMeta.getQtdDiariaCalorias(),
            planoMeta.getQtdDiariaProteinas(),
            planoMeta.getQtdDiariaCarboidratos(),
            planoMeta.getQtdDiariaGordura()
        );
    }
    ...
    public Long getQuantidadePlanosMetaPorEntidade() {
        Long entidade = authenticationFacade.getAuthentication().getEntidade();
        return planoMetaRepository.countByEntidadeId(entidade);
    }
